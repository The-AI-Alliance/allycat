"""
vison+text_multimodal_rag_service.py
...
def root(request: Request):
    # pass any variables to the template
    init_error = getattr(app.state, 'init_error', '')
    return templates.TemplateResponse("index.html", {"request": request, "init_error": init_error})
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run("vision+text_rag_model:app", host="0.0.0.0", port=11400, reload=True)
