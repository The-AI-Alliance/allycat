...
Run: uvicorn fancy_multimodal_rag_service:app
"""
...
def ensure_milvus_collection(name: str, dim: int = EMBED_DIM) -> Collection:
    ...
    if utility.has_collection(name):
        ...
        # (in production you should validate dim; here we assume it's compatible)
        try:
            col.load()
        except Exception:
            pass
...
class OllamaLLM:
    ...
    def __init__(self, model: str = OLLAMA_MODEL, host: str = OLLAMA_HOST, timeout: int = 60):
        self.model = model
        self.host = host
        # ollama python lib uses env OLLAMA_HOST or parameter; we keep it simple.
...
def root():
    ...
    <html>
    <head>
        <title>Allycat Vision RAG UI</title>
        <style>
...
The plant has multiple leaves that have brown spots of differing sizes and some holes in the leaves. Identify the plant and its illness. Tell me how to treat it.
            </textarea><br>
